...
export function MenuItemTile({ drink, delay = 0 }: { drink: Drink; delay?: number }) {
  ...
  return (
    <Reveal as="article" delay={delay} className="flex h-full flex-col">
      {/* Фото: большой квадрат, без скруглений и теней */}
      <div className="aspect-square overflow-hidden bg-[#f3f1ee]">
        <img
          src={drink.image}
          alt={drink.title}
          loading="lazy"
      ...
      <h3 className="product-title mt-6 text-2xl sm:text-[1.75rem]">{drink.title}</h3>

      {/* КБЖУ в одну строку: Б | Ж | У | К */}
      <dl
        aria-label="Пищевая ценность на порцию"
        className="mt-4 grid grid-cols-4 divide-x divide-border border-y border-border"
      >
        {MACROS.map((m) => (
          <div
            key={m.key}
          ...
          >
            <dt className="product-text-md text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
              {m.label}
            </dt>
      ...
      <div className="mt-4 flex items-baseline justify-between gap-4">
        <span className="product-text-md text-sm text-muted-foreground">{drink.size}</span>
        <span className="product-text-md text-base tabular-nums text-foreground">
          {drink.price}
        </span>
      </div>

      {/* Раскрывающийся блок: описание, состав, пищевая ценность */}
      <button
        type="button"
        aria-expanded={open}
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="mt-4">
              <p className="product-text-md text-[0.9375rem] text-foreground/80">
                {drink.description}
              </p>
              ...
              <div className="mt-6">
                ...
                <p className="product-text-md mt-1 text-sm text-foreground/80">
